...
from .simulator.instrument import Instrument
from .simulator.trade_context import ExecutionContext
from .simulator.scheduler import scheduler
from .utils import get_last_date
from .utils.data_structure import HybridDataFrame, missing_handler
from utils.const import EXECUTION_PHASE
...
def check_is_trading(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not bar_dict[order_book_id].is_trading:
            bt_log.error("Order Rejected: {order_book_id} is not trading.".format(
...
def order_shares(id_or_ins, amount, style=None):
    ...
    if amount == 0:
        bt_log.error("order_shares {order_book_id} amount is 0.".format(
            order_book_id=order_book_id,
        ))
        return
    ...
    bar_dict = ExecutionContext.get_current_bar_dict()
    order = get_trade_simulation().create_order(bar_dict, order_book_id, amount, style)
...
def order_value(id_or_ins, cash_amount, style=None):
    ...
    position = get_trade_simulation().account.portfolio.positions[order_book_id]
    if amount < 0:
...
def order_percent(id_or_ins, percent, style=None):
    ...
    portfolio_value = get_trade_simulation().account.portfolio.portfolio_value
...
def get_order(order_id):
    ...
    """
    return get_trade_simulation().get_order(order_id)
...
@ExecutionContext.enforce_phase(EXECUTION_PHASE.BEFORE_TRADING,
                                ...
                                EXECUTION_PHASE.SCHEDULED)
def get_open_orders():
    return copy.deepcopy(get_trade_simulation().open_orders)
...
def cancel_order(order_id):
    get_trade_simulation().cancel_order(order_id)
...
def get_trade_simulation():
    return ExecutionContext.get_trade_simulation()


def get_simulation_context():
    return ExecutionContext.get_simulation_context()


def get_simulation_executor():
    return ExecutionContext.get_simulation_executor()
...
def get_data_proxy():
    return ExecutionContext.get_simulation_executor().data_proxy
